...
addEventListener("keydown", function (event) {
  if (event.key === "b") {
    powerUpEnemyArr.push(new Boss(0));
    bossIsActive = true;
  }
});
  ...
  update() {
    ...
    for (let k = powerUpEnemyArr.length - 1; k >= 0; k--) {
      if (
        ...
        if (powerUpEnemyArr[k].isBoss) {
          enemySpawnChance = 70;
          powerUpSpawnChance = 55;
        }
...
class PowerUpEnemy {
  ...
  update(i) {
    ...
    if (this.r <= 10) {
      ...
      if (this.type == 32) {
        powerUpArr.push(new PowerUp(Math.floor(Math.random() * 7)));
        powerUpArr.push(new PowerUp(Math.floor(Math.random() * 7)));
      }
...
class Boss {
  ...
  shrink(damage) {
    ...
    if (this.type == 0) {
      this.damage -= damage / (2 + player.hp / 20);
    }
